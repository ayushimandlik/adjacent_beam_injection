...
# The following code has been taken from Vivek Gupta's code on obtaining the snr and width on top of Gaussian noise. This can be used if the usage of Heimdall seems diffult:
    ...
    tstart = (6000 - (4.14881e6 * DM_inj * ((middle_freq**-2 - 851.171875**-2)  / (327.68000*10**-3))) + 4500) * 327.68000*10**-3
    final = pd.DataFrame()
    if beam - int(beam) <= 0.5:
        final = final.append({'filterbank': values.high_res + '/BEAM_' + str(int(beam) -1).zfill(3) + '/' + values.utc + '.fil', 
            'snr': best_snr, 'tstart': tstart, 'dm': DM_inj, 'width': best_width, 'label': 1.0, 'beams': int(beam) -2, 'main_side': 'side'}, ignore_index = True)
    ...
    if beam - int(beam) > 0.5:
        ...
        final = final.append({'filterbank': values.high_res + '/BEAM_' + str(int(beam) +3).zfill(3) + '/' + values.utc + '.fil', 
            'snr': best_snr, 'tstart': tstart, 'dm': DM_inj, 'width': best_width, 'label': 1.0, 'beams': int(beam) +3, 'main_side': 'side'}, ignore_index = True)
...
#If the candidate parameter file is already made:
    ...
    a.add_argument('-m', '--molonglo_modules', help='Full path to molonglo modules file', type = str, default = 'molonglo_modules.txt')
    ...
    for index, row in cand_pars.iterrows():
        arguments.append([row['main_beam'], row['side_beam'], values.path_to_furby + '/furby_' + str(int(row['furby_id'])).zfill(4), 
            row['tstamp'], row['DM_inj'], row['SNR_inj'], values.output_directory, row['beam'], values.molonglo_modules])
    cands = pd.DataFrame()
