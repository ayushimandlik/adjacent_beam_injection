...
class Furby_reader:
    ...
    def read_header(self, filename):
        if not os.path.exists(filename):
            raise Furby_Error(
                "The furby file ({0}) does not exist".format(filename), 0)
        fil = open(filename, 'r')

        x = fil.read(16384).strip('\x00').strip()
        if "ID" not in x:
            raise Furby_Error(
                "File: '{0}' does not seem to have a valid furby header".format(self.filename), 0)
        h = x.split("\n")
        Header_tmp = {}
    ...
    def read_data(self, start=0, dd=False):
        ...
        dtypes = N.array(['float32', 'uint16', 'uint8'])
    ...
    def reshape_data(self, data):
        if self.header.ORDER == "TF":
            ...
            raise Furby_Error(
                "Unsupported ORDER in input : {0}".format(self.header.ORDER), 0)
        return d
    ...
    def dedisperse(self, data, dm=None):
        ...
        fch = (f0 + foff) + N.arange(self.header.NCHAN) * chw
